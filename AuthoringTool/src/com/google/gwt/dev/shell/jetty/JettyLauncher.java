...
public class JettyLauncher extends ServletContainerLauncher {
	...
	public static class JettyRequestLogger extends AbstractLifeCycle implements
		...
		public void log(Request request, Response response) {
			...
			if (logger.isLoggable(logStatus)) {
				TreeLogger branch = logger.branch(logStatus,
						String.valueOf(status) + " - " + request.getMethod()
								+ ' ' + request.getUri() + " (" + userString
	...
	protected static final class WebAppContextWithReload extends WebAppContext {
		...
		protected void doStop() throws Exception {
			...
			Class<?> jdbcUnloader = classLoader
					.loadClass("com.google.gwt.dev.shell.jetty.JDBCUnloader");
	...
	public boolean processArguments(TreeLogger logger, String arguments) {
		if (arguments != null && arguments.length() > 0) {
				...
				if ("ssl".equals(tag)) {
					...
					if (keyStoreUrl == null) {
						logger.log(TreeLogger.ERROR,
								"Default GWT keystore not found");
						return false;
					}
					...
					if (keyStorePassword == null) {
						logger.log(TreeLogger.ERROR,
								"Unable to read keystore password from '"
										+ value + "'");
						return false;
					}
					...
					try {
						...
						logger.log(TreeLogger.WARN,
								"Ignoring invalid clientAuth of '" + value
										+ "'");
					}
				} else {
			...
			if (useSsl) {
				if (keyStore == null) {
					logger.log(TreeLogger.ERROR,
							"A keystore is required to use SSL");
					return false;
				}
				if (keyStorePassword == null) {
					logger.log(TreeLogger.ERROR,
		...
		if (useSsl) {
			...
			if (clientAuth != null) {
				...
				case NONE:
					...
					break;
				case WANT:
					sslLogger.log(TreeLogger.TRACE,
	...
	private void jreLeakPrevention(TreeLogger logger) {
		...
		try {
			...
			logger.log(TreeLogger.ERROR,
					"jreLeakPrevention.jarUrlConnCacheFail", e);
		} catch (IOException e) {
			logger.log(TreeLogger.ERROR,
					"jreLeakPrevention.jarUrlConnCacheFail", e);
		}
